...
let AddMaintenanceComponent = class AddMaintenanceComponent {
    ...
    onItemSelectToStaff(item) {
        console.log(item);
        this.maintenance.maintainedbyStaff = item.id;
    }
    OnItemDeSelectToStaff(item) {
        console.log(item);
    }
    selectedItemsToStaff(selectedItems) {
        console.log(selectedItems);
    }
    onSelectAllToStaff(items) {
        console.log(items);
    }
    onDeSelectAllToStaff(items) {
        console.log(items);
    }
    uploadFile(event) {
...
let AssetsCreateComponent = class AssetsCreateComponent {
    ...
    ngOnInit() {
        ...
        if (this.route.params['value'].id != undefined) {
            ...
            this.assetService.getAllAssetByAssetId(params).subscribe((res) => {
                ...
                this.fileDetailsService.getFileDetailsById(newParams).subscribe((res) => {
                    this.filePath = res && res.length > 0 ? res[0].filePath : '';
                    this.downloadFile(this.filePath);
                });
            });
    ...
    getAllCategory() {
        ...
        this.lookupService.getSubcategory(queryParamBase).subscribe((res) => {
            if (res) {
                ...
                if (this.isEditAsset && this.asset.assetCategoryId) {
                    this.setSubCategory(this.asset.assetCategoryId);
                }
                else {
                    this.asset.assetCategoryId = this.categoryList && this.categoryList.length > 0 ? this.categoryList[0].id : '';
                    this.setSubCategory(this.asset.assetCategoryId);
                }
            }
        });
    ...
    setSubCategory(id) {
        if (this.categoryList && this.categoryList.length > 0) {
            this.categoryList.filter(val => {
                if (val.id == id) {
                    this.categorySubType = val.subCategory;
                }
            });
        }
    }
...
let AssetsSetupComponent = class AssetsSetupComponent {
    ...
    filterMaitenanceItem(value) {
        this.mainType = this.tempMainType.filter((item) => item.name.toLowerCase().indexOf(value.toLowerCase()) > -1);
    }
...
let LastMaintenanceComponent = class LastMaintenanceComponent {
    constructor(assetService, cookieService, lookupService, route, datePipe) {
        this.assetService = assetService;
        this.cookieService = cookieService;
        this.lookupService = lookupService;
        this.route = route;
    ...
    getMaintainAsset() {
        ...
        this.assetService.getAllAssetMaintenancePlanByApartmentIdStatus(params).subscribe((res) => {
            ...
            if (responeData && responeData.length > 0) {
                responeData.filter(val => {
                    val.actualMaintDate = val.actualMaintDate ? this.datePipe.transform(val.actualMaintDate, 'MM-dd-yyyy') : '';
                    val.plannedMaintenanceDate = val.plannedMaintenanceDate ? this.datePipe.transform(val.plannedMaintenanceDate, 'MM-dd-yyyy') : '';
                    val.nextPlannedMaintenance = val.nextPlannedMaintenance ? this.datePipe.transform(val.nextPlannedMaintenance, 'MM-dd-yyyy') : '';
                    val.lastMaintenanceDate = val.lastMaintenanceDate ? this.datePipe.transform(val.lastMaintenanceDate, 'MM-dd-yyyy') : '';
                });
            }
            this.gridSourceData = {
...
LastMaintenanceComponent = Object(tslib__WEBPACK_IMPORTED_MODULE_0__["__decorate"])([
    ...
    Object(tslib__WEBPACK_IMPORTED_MODULE_0__["__metadata"])("design:paramtypes", [src_app_api_controllers_Asset__WEBPACK_IMPORTED_MODULE_4__["AssetService"],
...
let MaintenanceHistoryComponent = class MaintenanceHistoryComponent {
    constructor(assetService, cookieService, lookupService, datePipe) {
        this.assetService = assetService;
        this.cookieService = cookieService;
        this.lookupService = lookupService;
        this.datePipe = datePipe;
    ...
    searchAsset() {
        ...
        if (this.assetName && this.categoryName) {
            ...
            filterArr.filter(val => {
                if (this.categoryName == val.assetCategoryName) {
        ...
        if (this.categoryName && this.subCategoryName) {
            filterCategoryArr.filter(val => {
                if (this.subCategoryName == val.assetSubcategoryName) {
...
/* harmony import */ var src_app_shared_services_shared_service__WEBPACK_IMPORTED_MODULE_6__ = __webpack_require__(/*! src/app/shared/services/shared.service */ "./src/app/shared/services/shared.service.ts");
/* harmony import */ var src_app_api_controllers_Lookup__WEBPACK_IMPORTED_MODULE_7__ = __webpack_require__(/*! src/app/api/controllers/Lookup */ "./src/app/api/controllers/Lookup.ts");
/* harmony import */ var src_app_shared_services_csv_service__WEBPACK_IMPORTED_MODULE_8__ = __webpack_require__(/*! src/app/shared/services/csv.service */ "./src/app/shared/services/csv.service.ts");
...
let ManageMaintenanceComponent = class ManageMaintenanceComponent {
    constructor(lookupService, sharedService, route, cookieService, injector, assetService, csvService) {
        this.lookupService = lookupService;
        this.sharedService = sharedService;
        this.route = route;
        this.cookieService = cookieService;
        ...
        this.sharedService.unitlistdeleteindexcast.subscribe(id => {
            if (id != null) {
                ...
                this.assetService.deleteAssetMaintenancePlan(param).subscribe((res) => {
                    ...
                    let pos = this.maintenanceList.map((e) => { return e.assetMaintenancePlanId; }).indexOf(id);
                    this.maintenanceList.splice(pos, 1);
                    this.totalItems = this.maintenanceList.length;
                    this.ItemEndIndex = this.totalItems;
                    //  if(this.totalItems>this.itemLimit){
                    //    this.ItemEndIndex = this.itemLimit;
    ...
    download() {
        let headerArray = ['assetTagNo', 'assetName', 'assetCategoryName',
            'assetSubcategoryName', 'actualMaintDate', 'maintenanceByCompany',
            'maintainedbyStaff', 'notes', 'maintLaborCost', 'maintPartsCost',
            'maintVat', 'maintWithTax', 'totalDiscount', 'maintenanceTotalBilledAmt',
            'plannedMaintenanceDate', 'nextPlannedMaintenance', 'alertBeforeDays'];
        this.csvService.downloadFile(this.maintenanceList, headerArray, 'manageMaintenance');
    }
    filterCategoryItem(value) {
        this.maintenanceList = this.tempMaintenanceList.filter((item) => item.assetName.toString().toLowerCase().indexOf(value.toLowerCase()) > -1
...
ManageMaintenanceComponent = Object(tslib__WEBPACK_IMPORTED_MODULE_0__["__decorate"])([
    ...
    Object(tslib__WEBPACK_IMPORTED_MODULE_0__["__metadata"])("design:paramtypes", [src_app_api_controllers_Lookup__WEBPACK_IMPORTED_MODULE_7__["LookupService"],
        ...
        _angular_core__WEBPACK_IMPORTED_MODULE_1__["Injector"],
        src_app_api_controllers_Asset__WEBPACK_IMPORTED_MODULE_4__["AssetService"],
        src_app_shared_services_csv_service__WEBPACK_IMPORTED_MODULE_8__["CsvService"]])
...
//# sourceMappingURL=ams-assets-assets-module-es2015.js.map