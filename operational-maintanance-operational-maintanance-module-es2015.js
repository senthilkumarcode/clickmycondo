...
let MaintainOperationalMaintananceComponent = class MaintainOperationalMaintananceComponent {
    ...
    ngOnInit() {
        ...
        if (this.opMaintenancePlanId) {
            ...
            this.genMaintService.getGenMaintenancePlanById(params).subscribe((res) => {
                ...
                this.dropDownListOfSubCategory();
            });
        }
    }
    uploadFile(event) {
    ...
    getAllStaff() {
        ...
        this.staffService.getAllStaffsByApartmentId(queryParamBase).subscribe((res) => {
            if (res.length > 0) {
                this.staffList.dropdownList = res.filter((ele) => {
                    ele.customLabel = `${ele.firstName} ${ele.lastName} ${ele.staffCategory_Label}`;
                    return ele.isActive;
                });
                if (this.isEdit) {
                    this.staffList.selectedItems = this.staffList.dropdownList.filter(data => data.staffId == parseInt(this.maintain.performedbyAssociationStaffId));
                }
            }
        });
    }
    addMaintanance() {
        ...
        reqObj = {
            ...
            "performedbyAssociationStaffId": this.staffList.selectedItems.length > 0 ? this.staffList.selectedItems[0].staffId : null
        };
        let params = {};
        ...
        this.genMaintService.addGenMaintenancePlan(params).subscribe((res) => {
            if (res.message) {
                ...
                this.router.navigate(['ams/operational-maintenance/operational-maintenance-list']);
            }
        });
    }
    updateMaintanance() {
    ...
    dropDownListOfSubCategory() {
        this.subCategoriesDropDown = this.subCategoriesData.filter(data => data.genMaintenanceCategoryId == this.maintain.categoryId);
    }
};
...
const routes = [
    ...
    { path: 'maintain-operational-maintenance', component: _maintain_operational_maintanance_maintain_operational_maintanance_component__WEBPACK_IMPORTED_MODULE_4__["MaintainOperationalMaintananceComponent"] },
    { path: 'maintain-operational-maintenance/:id', component: _maintain_operational_maintanance_maintain_operational_maintanance_component__WEBPACK_IMPORTED_MODULE_4__["MaintainOperationalMaintananceComponent"] },
    { path: 'operational-maintenance-list', component: _operational_maintanance_list_operational_maintanance_list_component__WEBPACK_IMPORTED_MODULE_5__["OperationalMaintananceListComponent"] },
    { path: 'operational-maintenance-history', component: _operational_maintanance_history_operational_maintanance_history_component__WEBPACK_IMPORTED_MODULE_6__["OperationalMaintananceHistoryComponent"] },
    { path: '**', redirectTo: 'setup', pathMatch: 'full' }
];
...
let OperationalMaintananceSetupComponent = class OperationalMaintananceSetupComponent {
    ...
    cancelCategory(index, type) {
        if (type == 'category') {
            this.categoriesData.splice(index, 1, this.currentData);
        }
        else {
            this.subCategoriesData.splice(index, 1, this.currentData);
        }
    }
};
